function model = VFFGPCR_train(X,Y,options)
...
% Initializing (W,gamma_var) and the corresponding Phi(necessary)
gamma_var = 10;
...
Phi = zeros(N,2*DF);
XW = X*W';
Phi(:,1:2:(2*DF)) = cos(XW)./realsqrt(DF);
Phi(:,2:2:(2*DF)) = sin(XW)./realsqrt(DF);
...
counter = 0;
ELBO_LB = [];
fprintf('Iter |ELBO_LB| \t xi \t\t gamma \t\t W \t\t muRHO \t\t mean_alpha \t mean_beta \t\t z| \t gamma \t\t mean(W) \t std(W) \t mean(muRHO) \t mean(z)\n');
fprintf('%i |%.7f| \t %.7f \t %.7f \t %.7f \t %.7f \t %.7f \t %.7f \t %.7f | \t %.7f \t %.7f \t %.7f \t %.7f \t %.7f \n',...
    [0,double(0),double(0),double(0),double(0),double(0),double(0),double(0),double(0)...
    gamma_var,mean(W(:)),std(W(:)),mean(muRHO),mean(z)])
for iter = 1:options.maxiter
    ...
    % Updating (W,sigma,gamma_var) and the corresponding Phi
    logGammaW = minimize([log(gamma_var);W(:)],@minus_posterior,-options.maxiter_par,X,v,g_xi);
    gamma_var = exp(logGammaW(1));
    W = reshape(logGammaW(2:end),DF,D);
    XW = X*W';
    Phi(:,1:2:(2*DF)) = cos(XW)./realsqrt(DF);
    Phi(:,2:2:(2*DF)) = sin(XW)./realsqrt(DF);
    ...
    fprintf('%i | %.7f | \t %.7f \t %.7f \t %.7f \t %.7f \t %.7f \t %.7f \t %.7f | \t %.7f \t %.7f \t %.7f \t %.7f \t %.7f \n',...
        [iter,ELBO_LB_i,xi_c,gamma_var_c,W_c,muRHO_c,mean_alpha_c,mean_beta_c,z_c...
        gamma_var,mean(W(:)),std(W(:)),mean(muRHO),mean(z)])
    ...
    elseif isequal(options.stopCriteria,'parameters')
        satisfiedStop = (xi_c<options.thr && gamma_var_c<options.thr && W_c<options.thr &&  ...
...
function [f, df] = minus_posterior(logGammaW,X,v,g_xi)
[N,D] = size(X);
df = zeros(size(logGammaW));
gamma_var = exp(logGammaW(1));
W = reshape(logGammaW(2:end),[],D);
DF = size(W,1);
Phi = zeros(N,2*DF);
XW = X*W';
Phi(:,1:2:(2*DF)) = cos(XW)./realsqrt(DF);
...
for i = 1:DF
    for j = 1:D
        dPhidWij = [-Phi(:,2*i),Phi(:,2*i-1)].*[X(:,j),X(:,j)];
        dfdW(i,j) = 2*sum(sum(aux3((2*i-1):(2*i),:)'.*dPhidWij))-...
            2*v'*dPhidWij*aux2((2*i-1):(2*i))+...
            2*aux4*dPhidWij*aux2((2*i-1):(2*i));
    end
end
df(2:end) = dfdW(:);
end
